...
export class WarningInfoPage implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.dataPass.orderData)
      if(this.dataPass.orderData.passengers)
        this.peopleNum = this.countOfChild(this.dataPass.orderData.passengers);

    this.dataPass.orderData.leaveDateText = this.leaveDate;
  ...
  goToPayment() {
    ...
    this.orderHistoryService.createOrder(this.dataPass.orderData, this.afAuth.auth.currentUser.uid)
    .then(() => {
      this.nav.navigateRoot("/payment/"+ ordernumber);
    });
  }
}
